(function () {
  ...
  $(document).ready(function () {
    $(document).off("click", ".btn-onayla").on("click", ".btn-onayla", function () {
      let satirElement = $(this).closest("tr, .rapor-row, .mobile-rapor-card"); // Butonun bulunduğu satırı/kartı al
  ...
  $(document).off("click", ".btn-kapat").on("click", ".btn-kapat", function () {
    let satirElement = $(this).closest("tr, .rapor-row, .mobile-rapor-card"); // Butonun bulunduğu satırı/kartı al
  ...
  $(document).off("click", ".btn-personel-degil").on("click", ".btn-personel-degil", function (e) {
    e.preventDefault();
    let satirElement = $(this).closest("tr, .rapor-row, .mobile-rapor-card"); // Butonun bulunduğu satırı/kartı al
    let raporData = satirElement.data("rapor"); // Satırdaki rapor verisini al
  ...
  $(document).ready(function () {
    ...
    const $raporSatirlari = $("tbody tr[data-gun-farki], .rapor-row[data-gun-farki], .mobile-rapor-card[data-gun-farki]");
    ...
    function filtreleRaporlari() {
      ...
      $raporSatirlari.each(function () {
        ...
        if (kisaRaporlariGoster || gunFarki >= 3) {
          ...
          if ($satir.is("tr, .rapor-row, .mobile-rapor-card")) {
            gosterilenSayisi++;
          }
        } else {
          // Değilse gizle
    ...
    $checkbox.off("change").on("change", filtreleRaporlari);
